...
class Conv1D_Block(nn.Module):
    ...
    def __init__(self, in_channels=256, out_channels=512,
                 ...
                 attn_dim=None,semantic_dim=None,fuse=None):
        ...
        self.causal = causal
        if fuse == "attention":
            self.adapter = GatedCrossAttnAdapter(
    ...
    def forward(self, x, semantic_feat):
        ...
        c = self.Sc_conv(c)
        # 语义注入
        c = self.adapter(c, semantic_feat)
        return x+c

class SeparationModule(nn.Module):
    def __init__(self, R, X, in_channels, out_channels,
             kernel_size, norm, causal, semantic_dim, attn_dim,fuse):
        super().__init__()
