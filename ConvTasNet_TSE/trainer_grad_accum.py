...
from Conv_TasNet_EncoderAtt import check_parameters
...
class Trainer():
    ...
    def __init__(self,
        ...
        if resume['resume_state']:
            cpt = torch.load(os.path.join(
                resume['path'], self.checkpoint, 'last.pt'), map_location='cpu')
    ...
    def create_optimizer(self, optimizer, kwargs, state=None):
        ...
        opt = supported_optimizer[optimizer](audio_params, **kwargs)
        # self.logger.info("第一阶段优化: 只训练文本编码器和Cross Attention模块")
        # self.logger.info(f"可训练参数: {len(text_cross_params)}组")
        # self.logger.info(f"冻结参数: {len(audio_params)}组")
        ...
        self.logger.info(f"可训练参数总数: {total_params}")
        # 查看各层参数
        for name, param in self.net.named_parameters():
            if param.requires_grad:
                print(f"{name}: {param.numel()} 个参数")
                self.logger.info(f"{name}: {param.numel()} 个参数")
    ...
    def train(self, train_dataloader):
        ...
        for egs in train_dataloader:
            ...
            # print("egs['ref'].shape",egs['ref'][0].shape,len(egs['ref']))
            ...
            loss = loss / self.accum_steps  # 归一化loss
            loss.backward()
            accum_counter += 1
    ...
    def val(self, val_dataloader):
        ...
        with torch.no_grad():
            for egs in val_dataloader:
                # print("egs['mix'].shape",egs['mix'].shape)
                # print("egs['target'].shape",egs['target'].shape,len(egs['target']))
                # print("len(val_dataloader)",len(val_dataloader))
                # print("current_step",current_step)
                current_step += 1
                # egs = to_device(egs, self.device)
